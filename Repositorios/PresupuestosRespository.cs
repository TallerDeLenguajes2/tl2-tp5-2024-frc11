...
class PresupuestosRepository{
    ...
    public List<Presupuestos> ListarPresupuestosGuardados(){
        ...
        string connectionString=@"DataSource=Tienda.db; Cache=Shared";
        string queryString=@"SELECT P.idPresupuesto, P.NombreDestinatario, P.FechaCreacion, PD.idProducto, PD.Descripcion, PD.Precio, D.Cantidad FROM Presupuestos P JOIN PresupuestosDetalles D ON P.idPrespuesto=D.idPresupuesto JOIN Productos PD ON D.idProducto=PD.idProducto ORDER BY P.idPresupuesto;";
        using(SqliteConnection connection=new SqliteConnection(connectionString)){
            connection.Open();
            SqliteCommand command=new SqliteCommand(queryString, connection);
            using(SqliteDataReader reader=command.ExecuteReader()){
                int id=-999;
                Presupuestos ultimo= new Presupuestos(-999, "", DateTime.Now);
                while(reader.Read()){
                    if(id==-999 || id!=Convert.ToInt32(reader["idPresupuesto"])){
                        if(id != -999){
